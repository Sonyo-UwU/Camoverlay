...
.ca-template-name:not([contenteditable]) {
    cursor: pointer;
}
.ca-template-right {
    align-items: center;
...
async function addAllianceButtonBack() {
  function addButton(node2) {
    const firstButton = node2.firstElementChild;
    ...
    node2.insertBefore(allianceButton, firstButton.title === "Store" ? firstButton.nextElementSibling : firstButton);
  }
    ...
    for (const mutation of mutationList)
      if (mutation.type === "childList") {
        for (const node2 of mutation.addedNodes)
          if (node2.classList.contains("flex"))
            addButton(node2);
      }
  });
...
function addListeners() {
  ...
  document.getElementById("ca-fly-hq").addEventListener("click", () => {
    ...
    Manager.flyToFit(topleft, bottomright.tx * 1e3 + bottomright.px - topleft.tx * 1e3 - topleft.px + 1, bottomright.ty * 1e3 + bottomright.py - topleft.ty * 1e3 - topleft.py + 1, 1);
  });
  function pasted(e) {
